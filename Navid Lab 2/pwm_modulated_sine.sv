...
module pwm_modulated_sine (
    ...
    output logic [1:0]   ARDUINO_IO,    // PWM outputs
...
// Parameters
//------------------------------------------------------------------------
localparam CLOCK_FREQ   = 50_000_000; // 50 MHz clock frequency
...
//------------------------------------------------------------------------
// Pending key event latches for KEY[0] and KEY[1]
...
always_ff @(posedge MAX10_CLK1_50) begin
    if (rst_sync) begin
        ...
        case (current_state)
            ...
            S_UPDATE: begin
                // Immediately update the frequency using the combinationally computed new_freq.
                freq <= new_freq;
