...
// Global variables
volatile unsigned int duty_cycle = 375;  // Start at 0° (1ms pulse width)
...
void execute_pwm_jobs(void) {
    if (state == 1) {  // Soft start (move servo towards 90°)
        if (duty_cycle < 562) {  // 562 corresponds to ~1.5ms pulse (90°)
            duty_cycle += 10;  // Increase pulse width
            PWMDTY1 = duty_cycle;
        } else {
        ...
        if (duty_cycle > 375) {  // 375 corresponds to ~1ms pulse (0°)
            duty_cycle -= 10;  // Decrease pulse width gradually
            PWMDTY1 = duty_cycle;
        } else {
            state = 1;  // Reset to soft start after stopping
            duty_cycle = 375;  // Ensure it stops at 0°
        }
    }
}
...
void pwm_init(void) {
    ...
    PWMPER1 = 7500;    // Period = 7500 ticks (~20ms for 50Hz)
